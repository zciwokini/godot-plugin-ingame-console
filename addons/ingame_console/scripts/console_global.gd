...
func _ready() -> void:
	...
	add_command(Command.new("quit", funcref(get_tree(), "quit"), [], "Terminates the application."))
	add_command(
		Command.new(
			# Command's name(id)
			"help",
			...
			[
				Arg.new(
					...
					"Lists information about a given command and it's usage. I.e.: [u]help command=quit[/u]"
				),
				Arg.new(
					"commands",
					[],
...
func log(category_id: String, text, level: String = "") -> void:
	...
	if category_id in log_categories:
...
func list_commands() -> void:
	...
	for command_id in command_ids:
		...
		content += "[cell]%s[/cell][cell]%s[/cell]\n" % [command.id, command.short_description]
	write_line("[indent][table=2]%s[/table][/indent]\n" % content)


func list_command_info(command_id) -> void:
	...
	if not command.arguments.empty():
		...
		for argument in command.arguments:
			content += "[cell]%s[/cell][cell]%s[/cell]" % [argument.id, argument.description]
		write_line("[indent][table=2]%s[/table][/indent]\n" % content)


func clear_console(args) -> void:
